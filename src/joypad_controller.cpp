...
JoypadController::JoypadController(UI * _ui) noexcept
{
	...
	Log.write("Create JoypadController" , cLog::LOG_TYPE::L_INFO); 
}
...
JoypadController::~JoypadController()
{
	...
	Log.write("delete JoypadController", cLog::LOG_TYPE::L_INFO); 
...
void JoypadController::init(const std::string &_configName) noexcept
{
	...
	string model = SDL_JoystickName(joystick);

	//~ cout << model << " connected" << endl;
	Log.write("Joypad: model name "+ model + " detected", cLog::LOG_TYPE::L_INFO);  

	if(!conf.findEntry(model)) {
		model = "joy_default";
		//~ cout << "no config detected switching to default" << endl;
		Log.write("Joypad: no config detected switching to default", cLog::LOG_TYPE::L_WARNING);  
	} else {
		//~ cout << "config detected"<<endl;
		Log.write("Joypad: config detected", cLog::LOG_TYPE::L_INFO);  
	}

	for (int i = 0; i < nbrAxis; i++) {
		string actionStr = conf.getStr(model,"AXIS" + to_string(i));
		axisActions[i] = getAxisActionFromString(actionStr);
		axisDeadZone[i] = conf.getDouble(model,"AXIS" + to_string(i) + "_DEADZONE");
		axisSensitivity[i] = conf.getDouble(model,"AXIS" + to_string(i) + "_SENSITIVITY");
		axisIsStick[i] = conf.getBoolean(model,"AXIS" + to_string(i) + "_IS_STICK");
	}

	for (int i = 0; i < nbrButtons; i++) {
		string actionStr = conf.getStr(model,"BUTTON" + to_string(i));
		buttonActions[i] = getButtonActionFromString(actionStr, i);
	}

	for(int i = 0; i < nbrHats; i++) {
		string actionStr = conf.getStr(model,"HAT" + to_string(i) + "_DOWN");
		hatActions[i + (int)(hat_event::hat_down)] = getButtonActionFromString(actionStr, -1);

		actionStr = conf.getStr(model,"HAT" + to_string(i) + "_UP");
		hatActions[i + (int)(hat_event::hat_up)] = getButtonActionFromString(actionStr, -1);

		actionStr = conf.getStr(model,"HAT" + to_string(i) + "_LEFT");
		hatActions[i + (int)(hat_event::hat_left)] = getButtonActionFromString(actionStr, -1);

		actionStr = conf.getStr(model,"HAT" + to_string(i) + "_RIGHT");
...
void JoypadController::handleJoyButtonDown(const SDL_JoyButtonEvent &E) noexcept
{
	...
	if(isCommand(E.button)) {
		for(string command : buttonCommand[E.button]) {
			cout << command << endl;
...
joy_button_action JoypadController::getButtonActionFromString(std::string &actionStr, int buttonNumber) noexcept
{
	...
	if(actionStr[actionStr.size()-1] == ')') { // It's a command
		...
		actionStr.pop_back();
		vector<string> commands_raw;
		split(actionStr, ",", commands_raw);

		vector<string> commands;

		for(string s : commands_raw) {
			s = s.substr(1);
			s.pop_back();
			commands.push_back(s);
		}

		buttonCommand.insert(pair<Uint8, vector<string>>(buttonNumber, commands));	//add the commands to call in dictionnary
