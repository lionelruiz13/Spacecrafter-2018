...
Audio::Audio()
{
	...
	elapsed_seconds=0.0;
	Log.write("Audio initialization successful", cLog::LOG_TYPE::L_INFO);
...
Audio::~Audio()
{
	...
	Mix_CloseAudio();
	Log.write("Audio support end", cLog::LOG_TYPE::L_INFO);
}
...
void Audio::musicLoad(const std::string& filename)
{
	...
	if (track == nullptr) {
		music_isPlaying = false;
		Log.write("Could not load audio file " +filename, cLog::LOG_TYPE::L_WARNING);
...
void Audio::chunkLoad(const std::string& filename)
{
	for (int i=0; i<MAX_CHANNELS; i++) {
		if(sound[i] ==nullptr) {
			...
			if (sound[i] == nullptr) {
				Log.write("Nothing to do because no existing sound with this name", cLog::LOG_TYPE::L_ERROR);
				return;
			}
	...
	Log.write("Nothing to do because no channel free", cLog::LOG_TYPE::L_WARNING);
}
...
void Audio::chunkDrop(const std::string& filename)
{
	...
	Log.write("Nothing to do because no existing sound with this name", cLog::LOG_TYPE::L_WARNING);
}
