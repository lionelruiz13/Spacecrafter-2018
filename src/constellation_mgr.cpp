...
void ConstellationMgr::setLineColor(const std::string _name, const Vec3f& c)
{
	...
	Log.write("No constellation Color with shortName "+_name,cLog::LOG_TYPE::L_WARNING, cLog::cLog::LOG_FILE::SCRIPT );
}
...
void ConstellationMgr::setLabelColor(const std::string _name, const Vec3f& c)
{
	...
	Log.write("No constellation label with shortName "+_name,cLog::LOG_TYPE::L_WARNING, cLog::cLog::LOG_FILE::SCRIPT );
}
...
int ConstellationMgr::loadLinesAndArt(const string &skyCultureDir)
{
	...
	if (!inf.is_open()) {
		Log.write("Constellation_Mgr::loadLinesAndArt can't open constellation data file "+ fileName, cLog::LOG_TYPE::L_ERROR);
		return -1;
	}
	...
	while (!inf.eof() && std::getline(inf, record)) {
		...
		if (cons->read(record, hipStarMgr)) {
			...
			Log.write("ConstellationMgr::loadLinesAndArt on line " + Utility::intToString(line) + " of " + fileName, cLog::LOG_TYPE::L_ERROR);
			delete cons;
		}
	}
	inf.close();
	...
	if (!fic) {
		Log.write("ConstellationMgr::loadLinesAndArt Can't open " + artfileName, cLog::LOG_TYPE::L_ERROR);
	...
	while (!artFile.eof() && std::getline(artFile, record)) {
		...
		if ( record != "") {
			...
			if(in.fail()) {
				...
				Log.write("ConstellationMgr::loadLinesAndArt Error parsing constellation art record " + record, cLog::LOG_TYPE::L_ERROR);
				continue;
			}
			...
			if (!cons) {
				...
				Log.write("ConstellationMgr::loadLinesAndArt can't find constellation called : " + shortname, cLog::LOG_TYPE::L_ERROR);
			} else {
...
{
	vector<float> vLinesPos;
	vector<float> vLinesColor;
...
void ConstellationMgr::loadNames(const string& namesFile)
{
	...
	if (!commonNameFile.is_open()) {
		...
		Log.write("ConstellationMgr::loadNames :: Can't open file " + namesFile, cLog::LOG_TYPE::L_ERROR);
		return;
	}
...
void ConstellationMgr::setArtIntensity(const std::string &_name, float _max)
{
	...
	Log.write("No constellation ArtIntensity with shortName "+_name,cLog::LOG_TYPE::L_WARNING, cLog::cLog::LOG_FILE::SCRIPT );
}
...
void ConstellationMgr::setSelectedConst(Constellation * c)
{
	...
	if (c != nullptr) {
		...
		bool matchc = 0;
		vector < Constellation * >::const_iterator c_iter;
...
bool ConstellationMgr::loadBoundaries(const string& boundaryFile)
{
	...
	if (!dataFile.is_open()) {
		Log.write("Boundary file " + boundaryFile + " not found", cLog::LOG_TYPE::L_ERROR);
		return false;
	}

	Log.write("Loading Constellation boundary data...", cLog::LOG_TYPE::L_INFO);

	float DE, RA;
	Vec3f XYZ;
	unsigned num, numc;
	vector<Vec3f> *points = nullptr;
	string consname;
	i = 0;
	while (!dataFile.eof()) {
	...
	dataFile.close();
	Log.write("(" + Utility::intToString(i) + " segments loaded)", cLog::LOG_TYPE::L_INFO);
	delete points;
...
{
	vector<float> vBoundariesPos;
	vector<float> vBoundariesIntensity;
