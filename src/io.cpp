...
void ServerSocket::setOutput(std::string data)
{
	if(lock(outputting) == IO_NO_ERROR) {
		...
		if (sz>MAX_BUFFER) {
			Log.write("ServerSocket data setOutput too big", cLog::LOG_TYPE::L_WARNING);
...
bool ServerSocket::debug(int level, int scope)
{
	if(logLevel >= level && logScope >= scope) {
		switch(level) {
			case IO_DEBUG_FATAL :
				logType = cLog::LOG_TYPE::L_ERROR;
				break;
			case IO_DEBUG_ERROR :
				logType = cLog::LOG_TYPE::L_ERROR;
				break;
			case IO_DEBUG_WARN :
				logType = cLog::LOG_TYPE::L_WARNING;
				break;
			case IO_DEBUG_INFO :
				logType = cLog::LOG_TYPE::L_INFO;
				break;
			case IO_DEBUG_DEBUG :
				logType = cLog::LOG_TYPE::L_DEBUG;
				break;
			case IO_DEBUG_TRACE :
				logType = cLog::LOG_TYPE::L_DEBUG;
				break;
...
{
	Log.write("TCP : " + msg, logType, cLog::LOG_FILE::TCP);
}
