...
void shaderProgram::debugShader( GLuint shader, const std::string &str)
{
	...
	if(compile_status != GL_TRUE) {
		...
		cout<<"impossible de compiler le shader : "<< shader << " " << str << endl<<log<<endl;
		free(log);
...
void shaderProgram::debugProgram()
{
	...
	if (isLinked == GL_FALSE)
	{
		...
		cout<<"impossible de compiler le program : "<< program << " " << programName << endl<<log<<endl;
		free(log);
		...
		//~ cout<<"program : "<< program << " name " << programName << endl;
	}
}
...
{
	string fullStr = shaderDir + str;
	string shader_string = loadFileToString(fullStr.c_str());
