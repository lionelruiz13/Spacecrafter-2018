...
string SpaceDate::getPrintableTimeNav(double jd, double longitude, double latitude) const
{
	ostringstream os;
	double sidereal;
	double T;
...
string SpaceDate::TimeZoneNameFromSystem(double JD)
{
...
//-----------------------------------------------------------------------------
double SpaceDate::JulianDayFromDateTime(const int y, const int m, const int d,
...
// to julian day
int SpaceDate::StringToJday(string date, double &rjd)
{
	char tmp;
...
string SpaceDate::ISO8601TimeUTC(double jd, bool dateOnly)
{
	int year, month, day, hour, minute;
	double second;
...
string SpaceDate::getPrintableDateUTC(double JD) const
{
	struct tm time_utc;
...
string SpaceDate::getPrintableDateLocal(double JD) const
{
...
string SpaceDate::getPrintableTimeUTC(double JD) const
{
	struct tm time_utc;
	SpaceDate::TimeTmFromJulian(JD, &time_utc);
...
string SpaceDate::getPrintableTimeLocal(double JD) const
{
	struct tm time_local;
...
void SpaceDate::setCustomTzName(const string& tzname)
{
	#if LINUX
...
string SpaceDate::getISO8601TimeLocal(double JD) const
{
	struct tm time_local;
	if (timeZoneMode == S_TZ_GMT_SHIFT)
		SpaceDate::TimeTmFromJulian(JD + GMTShift, &time_local);
...
SpaceDate::S_TIME_FORMAT SpaceDate::stringToSTimeFormat(const string& tf) const
{
	if (tf == "system_default") return S_TIME_SYSTEM_DEFAULT;
	if (tf == "24h") return S_TIME_24H;
	if (tf == "12h") return S_TIME_12H;
	Log.write(" unrecognized time_display_format : " + tf + " system_default used.", cLog::LOG_TYPE::L_ERROR);
	return S_TIME_SYSTEM_DEFAULT;
}

//! Convert string to its associated time format enum
string SpaceDate::sTimeFormatToString(S_TIME_FORMAT tf) const
{
...
SpaceDate::S_DATE_FORMAT SpaceDate::stringToSDateFormat(const string& df) const
{
	...
	Log.write(" unrecognized date_display_format : " + df + " system_default used.",cLog::LOG_TYPE::L_ERROR );
	return S_DATE_SYSTEM_DEFAULT;
}

//! Convert string to its associated date format enum
string SpaceDate::sDateFormatToString(S_DATE_FORMAT df) const
{
