...
static const std::string startTui("");//cyp utilisée 1 seule fois
static const std::string endTui("");//cyp utilisée 1 seule fois
...
public:
	...
	virtual std::string getCleanString(void);
...
public:
	...
	std::string getCleanString(void) {
		return label + value;
	}
...
public:
	...
	virtual bool onKey(SDL_Scancode, S_TUI_VALUE);
	virtual std::string getString(void);
	virtual std::string getDateString(void);
	virtual bool isEditable(void) const {
		return true;
	}
...
template<>
std::string MultiSetItem<std::string>::getString(void);
...
public:
	...
	std::string gettz(void); // should be const but gives a boring error...
