...
MilkyWay::MilkyWay()
{
	...
	if (!sphere->getOk()) {
		Log.write("MilkyWay : error loading sphere, no draw", cLog::LOG_TYPE::L_ERROR);
		return;
	} else {
		Log.write("MilkyWay : successful loading sphere", cLog::LOG_TYPE::L_INFO);
		isDraw = true;
	}
...
void MilkyWay::defineZodiacalState(const std::string& tex_file, float _intensity)
{
	if (zodiacal.tex==nullptr) { //fist time to read this texture
		...
		zodiacal.intensity = clamp(_intensity, 0.f, 1.f);
		zodiacal.name = tex_file;
	} else {
		Log.write("Milkyway: zodicalState already exist, function aborded" , cLog::LOG_TYPE::L_WARNING);
	}
}
...
void MilkyWay::defineInitialMilkywayState(const std::string& path_file,const std::string& tex_file, const std::string& iris_tex_file, float _intensity)
{
	if (defaultMilky.tex==nullptr) {
		...
		if (!iris_tex_file.empty()) {
			...
			irisMilky.intensity =  clamp(_intensity, 0.f, 1.f);
			irisMilky.name = path_file + iris_tex_file;
			Log.write("Milkyway: define irisMilky, name "+ iris_tex_file, cLog::LOG_TYPE::L_DEBUG);
		} else
			Log.write("Milkyway: no irisMilky define" , cLog::LOG_TYPE::L_DEBUG);
	} else {
		Log.write("Initial textures already define, function aborded", cLog::LOG_TYPE::L_WARNING);
	}
}
