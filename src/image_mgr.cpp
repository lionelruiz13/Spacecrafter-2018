...
{
	for (list<Image*>::iterator iter = active_images.begin(); iter != active_images.end(); ++iter) {
...
void ImageMgr::setLocation(float xpos, bool deltax, float ypos, bool deltay, float duration) {
	if (currentImg != nullptr)
		currentImg -> setLocation(xpos, deltax, ypos, deltay, duration);
}
