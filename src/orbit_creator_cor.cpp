...
Orbit * OrbitCreatorEliptic::handle(stringHash_t params) const{
	...
	if(params["coord_func"] != "ell_orbit"){
		if(next != nullptr)
			...
			Log.write("OrbitCreatorEliptic::handle unknown type");
			return nullptr;
		}
	}
...
Orbit * OrbitCreatorComet::handle(stringHash_t params) const{
	...
	if(params["coord_func"] != "comet_orbit"){
		if(next != nullptr)
			...
			Log.write("OrbitCreatorComet::handle unknown type : " + params["coord_func"]);
			return nullptr;
		}
	}
	...
	if (pericenter_distance <= 0.0) {
		...
		if (semi_major_axis <= -1e100) {
			Log.write("OrbitCreatorComet::handle you must provide orbit_pericenterdistance or orbit_semimajoraxis");
			return nullptr;
		} else {
			if (eccentricity == 1.0){
				Log.write("OrbitCreatorComet::handle parabolic orbits have no semi_major_axis");
				return nullptr;
			}
	...
	if (mean_motion <= -1e100) {
		...
		if (period <= -1e100) {
			if (parent->get_parent()){				
				Log.write("OrbitCreatorComet::handle When the parent body is not the Sun\nyou must provide orbit_MeanMotion or orbit_Period");
				return nullptr;
			}
	...
	if (time_at_pericenter <= -1e100) {
		...
		if (epoch <= -1e100 || mean_anomaly <= -1e100) {
			Log.write("OrbitCreatorComet::handle when you do not provide orbit_TimeAtPericenter, you must provide both orbit_Epoch and orbit_MeanAnomaly");
			return nullptr;
		} else {
...
Orbit * OrbitCreatorSpecial::handle(stringHash_t params) const{
	...
	if(!sorb->isValid()){
		if(next != nullptr)
			...
			Log.write("OrbitCreatorComet::handle unknown type");
			return nullptr;
		}
	}
...
Orbit * OrbitCreatorBary::handle(stringHash_t params) const{
	...
	if(params["coord_func"] != "barycenter"){
		if(next != nullptr)
			...
			Log.write("OrbitCreatorBary::handle unknown type");
			return nullptr;
		}
	}
	
	if(params["a"].empty() || params["b"].empty()){
		Log.write("OrbitCreatorBary::handle missing barycenter coefficients");
		return nullptr;
	}
	
	if(params["body_A"].empty() || params["body_B"].empty()){
		Log.write("OrbitCreatorBary::handle missing parents");
		return nullptr;
	}
	...
	if(bodyA == nullptr || bodyB == nullptr){
		Log.write("OrbitCreatorBary::couldn't find one of the bodies");
		return nullptr;
	}
