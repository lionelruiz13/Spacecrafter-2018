...
Nebula::Nebula(string _englishName, string _mtype, string _constellation, float _ra, float _de, float _mag, float _size, string _classe,
               float _distance, string tex_name, bool path, float tex_angular_size, float tex_rotation, string tex_credit, float _luminance, bool _deletable, bool _hidden) :
	neb_tex(nullptr)
{
	...
	if (tex_credit  == "none")
		...
		credit = string("Credit: ") + tex_credit;
...
nebula_type Nebula::getDsoType( string type)
{
	if (type == "GALXY")
		return GALXY;
	else if (type == "OPNCL")
...
string Nebula::getInfoString(const Navigator* nav) const
{
	...
	oss << _("Type: ") << getTypeToString() << endl;
	oss << _("Size: ") << Utility::printAngleDMS(m_angular_size*C_PI/180.) << endl;
	return oss.str();
}
...
string Nebula::getShortInfoNavString(const Navigator*, const TimeMgr * timeMgr, const Observer* observatory) const
{
	return "";
}
...
void Nebula::drawHint(const Projector* prj, const Navigator * nav, vector<float> &vecHintPos, vector<float> &vecHintTex, vector<float> &vecHintColor)
{
...
string Nebula::getTypeToString(void) const
{
	switch (DSOType) {
		case GALXY:
			return _("Galaxy");
