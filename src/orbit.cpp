...
string EllipticalOrbit::saveOrbit() const{
	
	ostringstream os;
	
	os << "coord_func = ell_orbit" << endl;
	os << "orbit_period = " << period << endl;
	os << "orbit_epoch = " << epoch << endl;
	os << "orbit_eccentricity = " << eccentricity << endl;
	os << "orbit_inclination = " << inclination / C_PI*180. << endl;
	os << "orbit_ascendingnode = " << ascendingNode / C_PI*180. << endl;
	...
	os << "orbit_longofpericenter = " << long_of_pericenter / C_PI*180. << endl;
	os << "orbit_meanlongitude = " << mean_longitude/ C_PI * 180 << endl;
	os << "orbit_semimajoraxis = " << semi_major_axis * AU << endl;
...
string CometOrbit::saveOrbit() const{
	/*
	ostringstream os;
...
string BinaryOrbit::saveOrbit() const{
	
	return "Pas encore complet";
}

string MixedOrbit::saveOrbit() const{
	
	return "Pas encore complet";
}

string SpecialOrbit::saveOrbit() const{
	
	return "Pas encore complet";
}

string linearOrbit::saveOrbit() const{
	
	return "Pas encore complet";
}

string stillOrbit::saveOrbit() const{
        ...
        bool useParentPrecession)
{
	...
	// Compute the period
	//double T = 2 * C_PI * sqrt((a*a*a) / GM);
	//T = T / 86400.0; // Convert from seconds to days
	//	double a2 = pow(GM * pow(86400*T/2.0/C_PI, 2), 1.0/3.0);
...
std::string BarycenterOrbit::saveOrbit() const{
	...
	os << "coord_func = barycenter" << endl;
	os << "body_A = " << bodyA->getEnglishName() << endl;
	os << "body_B = " << bodyB->getEnglishName() << endl;
	os << "a = " << a << endl;
	os << "b = " << b << endl;
