...
int Media::playerPlay(const std::string &type, const std::string &filename, const std::string& _name, const std::string& _position)
{
	...
	Log.write("Media::playerPlay trying to play videofilename "+filename, cLog::LOG_TYPE::L_DEBUG);
	int tmp = player->play(filename);
	if (tmp !=0) {
		Log.write("Media::playerPlay error playing videofilename "+filename, cLog::LOG_TYPE::L_ERROR);
		return tmp;
	}
	...
	if (playerGetAlive()) {
		...
		mediaState.video_state=V_STATE::V_OFF;
		mediaState.video_type=V_TYPE::V_NO;
		Log.write("Media::playerPlay error playerVideo with "+filename, cLog::LOG_TYPE::L_ERROR);
...
int Media::playerPlay(const std::string &type,const  std::string &videoname, const std::string &audioname, const std::string& _name, const std::string& _position)
{
	...
	Log.write("Media::playerPlay trying to play videofilename "+videoname, cLog::LOG_TYPE::L_DEBUG);
	Log.write("Media::playerPlay trying to play audiofilename "+audioname, cLog::LOG_TYPE::L_DEBUG);
...
void Media::playerStop()
{
	...
	Log.write("Media::playerPlayStop", cLog::LOG_TYPE::L_INFO);
	player->playStop();
	mediaState.video_state=V_STATE::V_OFF;
	audio->musicDrop();
...
void Media::playerRestart()
{
	...
	Log.write("Media::playerRestart", cLog::LOG_TYPE::L_INFO);
	player->RestartVideo();
	audio->musicRewind();
}
...
void Media::createVR360(ScriptMgr *_scriptMgr)
{
	vr360 = new VR360(_scriptMgr);
}
