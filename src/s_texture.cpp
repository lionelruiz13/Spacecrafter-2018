...
s_texture::s_texture(const s_texture *t)
{
	...
	if (it != texCache.end()) {
		...
		Log.write("s_texture: erreur de duplication " + textureName , cLog::LOG_TYPE::L_ERROR);
	}
}
...
bool s_texture::load(std::string fullName, bool mipmap)
{
	...
	if (it != texCache.end()) { // texture existe deja
		...
		try {
			...
			// NPOT check
			if ((x & (x - 1)) != 0 || (y & (y - 1)) != 0) {
				Log.write("s_texture: not power-of-2 dimensions for " + fullName , cLog::LOG_TYPE::L_WARNING);
...
float s_texture::getAverageLuminance(void) const
{
	glBindTexture(GL_TEXTURE_2D, texID);
	GLint w, h;
	glGetTexLevelParameteriv(GL_TEXTURE_2D, 0, GL_TEXTURE_WIDTH, &w);
