...
void Sun::setBigHalo(const string& halotexfile, const string &path)
{
...
{
	//~ AutoPerfDebug apd(&pd, "Sun::drawBigHalo$"); //Debug
...
{
	//~ AutoPerfDebug apd(&pd, "Sun::draw$"); //Debug
	//~ if (hidden) return ; //0;
...
bool Sun::drawGL(Projector* prj, const Navigator* nav, const Observer* observatory, const ToneReproductor* eye, bool depthTest, bool drawHomePlanet, bool selected)
{
	...
	//~ if (ang_dist>0.25) //{  //on le trace tout le temps
...
{
	//~ AutoPerfDebug apd(&pd, "Sun::drawBody$"); //Debug
